...
func search(bTable map[string]uint64, id string, as []string) uint64 {
	if len(as) == 0 {
		return 0
	} else if len(as) == 1 {
		if id >= as[0] {
	...
	if m == id {
		...
		rightV := search(bTable, id, as[len(as)/2:])
		return uint64(max(int(leftV), int(rightV)))
	} else {
		return search(bTable, id, as[0:len(as)/2])
	}
}
