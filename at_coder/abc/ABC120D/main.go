...
func main() {
	...
	nodeCount := makeInts(N+1, 1)
	ans := make([]int64, M)
	ans[M-1] = int64((N * (N - 1)) / 2) // 不便さの最大値=ペアの数
	uf := NewUnionFind(N + 1)
	for i := M - 1; i > 0; i-- {
		a := as[i]
		b := bs[i]
		if uf.root(a) == uf.root(b) {
			ans[i-1] = ans[i] // 既に連結されているので不便さは減らない
		} else {
			// 分かれているグラフg1, g2が連結すると、count(g1)*count(g2)のペアができる
			// -> 不便さがcount(g1)*count(g2)だけ減る
			ans[i-1] = ans[i] - int64(nodeCount[uf.root(a)]*nodeCount[uf.root(b)])
			sum := nodeCount[uf.root(a)] + nodeCount[uf.root(b)]
			uf.Unite(a, b)
			nodeCount[uf.root(a)] = sum // 連結後のノード数
		}
	}
	for i := 0; i < M; i++ {
		fmt.Println(ans[i])
