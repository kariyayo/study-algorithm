...
// cf. https://drken1215.hatenablog.com/entry/2020/05/18/174000
func main() {
	...
	// 木の累積和
	// DFSで根から子へ値を配っていく
	dfs(G, 0, func(node int, x int) {
		counter[node] = counter[node] + x
	})

	ans := make([]string, N)
	for i := 0; i < N; i++ {
		ans[i] = strconv.Itoa(counter[i])
	}
...
func dfs(G Graph, start int, f func(int, int)) {
	size := len(G)
...
func NewStack() *Stack {
	return &Stack{xs: make([]int, 1010100)}
}
...
func NewQueue() *Queue {
	return &Queue{xs: make([]int, 1010100)}
}
