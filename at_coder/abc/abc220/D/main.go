...
func main() {
	...
	for i := 1; i < N; i++ {
		var k int64
		for k = 0; k < 10; k++ {
			dp[i][F(k, as[i])] = dp[i][F(k, as[i])] + dp[i-1][k]
			dp[i][G(k, as[i])] = dp[i][G(k, as[i])] + dp[i-1][k]
		}
		for k = 0; k < 10; k++ {
			dp[i][F(k, as[i])] = dp[i][F(k, as[i])] % 998244353
			dp[i][G(k, as[i])] = dp[i][G(k, as[i])] % 998244353
		}
	}
	var k int64
	for k = 0; k < 10; k++ {
